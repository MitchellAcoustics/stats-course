...
pA <- ggplot(poverty, aes(x = Poverty, y = Graduates)) +
  geom_point() +
  labs(x = "Poverty Rate", y = "Graduation Rate") +
  theme_bw()
...
pB <- ggplot(poverty_shuffled, aes(x = Poverty, y = Graduates)) +
  geom_point() +
  labs(x = "Poverty Rate", y = "Graduation Rate") +
  theme_bw()
...
p_real_scatter <- poverty %>%
  ggplot(aes(x = Poverty, y = Graduates)) +
  annotate(
    "rect",
    xmin = 5,
    xmax = 9,
  ...
  geom_point() +
  geom_rug(data = poverty_2, aes(color = group), alpha = .8, sides = "r") +
  theme_bw() +
  theme(legend.position = "none")
...
ggplot(poverty, aes(x = Poverty, y = Graduates)) +
  geom_point() +
...
poverty %>%
  ...
  geom_abline(slope = coef(m1)[2], intercept = coef(m1)[1], color = "blue") +
  labs(x = "Poverty Rate", y = "Graduation Rate") +
  theme_bw()
```
...
` and `
residuals()
` return vectors. To match the vectors up with the observations, we can *mutate* them as columns onto the original data frame. From here, we can isolate the residual for the state of California as before.

```{r}
#| code-fold: false
poverty |>
  mutate(y_hat = fitted(m1), e_hat = residuals(m1)) |>
