...
#| fig-align: center
#| echo: false
pop_eager <- data.frame(
  year = rep(c(1, 2, 3, 4), times = c(245, 210, 47, 25)),
  eagerness = rep(c(10, 6, 3, 1), times = c(245, 210, 47, 25))
)
set.seed(55)
pop_eager <- pop_eager %>%
...
p_pop <- pop_eager %>%
  ...
  labs(title = "Population Distribution", y = "") +
  annotate(
    "point",
    x = mu,
    y = -20,
...
fs_scores <- read_csv("data/fs_scores.csv")
fs_scores <- fs_scores$scores

pop_dist <- ggplot(
  as.data.frame(fs_scores),
  aes(x = fs_scores, y = after_stat(density))
) +
  geom_histogram(
    breaks = seq(47.5, 100.5, by = 1),
    color = "black",
    fill = "gray"
  ) +
...
sampling_dist <- ggplot(
  as.data.frame(samp_means),
  aes(x = samp_means, y = after_stat(density))
) +
  geom_histogram(bins = 45, color = "black", fill = "gray") +
...
ci100 <- ggplot(CIs, aes(y = trial)) +
  geom_segment(
    aes(x = lower, y = trial, xend = upper, yend = trial, color = cover),
    show.legend = FALSE
  ) +
  annotate(
    "segment",
...
hi_score_dist <- ggplot(
  as.data.frame(hi_scores),
...
hi_samp_dist <- ggplot(
  as.data.frame(hi_score_samp),
  aes(x = hi_score_samp, y = ..density..)
) +
  geom_histogram(
    breaks = seq(-0.5, 1.5, by = 1),
    color = "black",
...
hi_sampling_dist <- ggplot(
  as.data.frame(hi_score_means),
  aes(x = hi_score_means, y = ..density..)
) +
  geom_histogram(bins = 16, color = "black", fill = "gray") +
